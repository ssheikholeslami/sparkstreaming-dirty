/**
  ...
  *
  * A simple Spark Streaming program that prints and aggregates input elements in intervals of 10 seconds
...
object IntervalAggregator {
  ...
  var intervalCount = 1
  // FIXME should use accumulators! , cluster mode consideration
  var elementCount = 0
  // FIXME should use accumulators! , cluster mode consideration
  var sum = 0 // FIXME should use accumulators! , cluster mode consideration
  ...
  def main(args: Array[String]) {
    ...
    lines.foreachRDD { (rdd, time) =>
      ...
      rdd.foreach(element => {
        ...
        sum = sum + element.toInt
      }
      )
      println("** sum of elements in this batch: " + sum)
